...
def cb_target(self, context):
    ...
    inst.direct = inst.direct_u * ob.MKVBS.learning_rate
    return
...
def oneStep_PDiff(inst):
    ...
    idx = 0
    for v in base_ob.data.shape_keys.key_blocks[inst.shapekey_n].data:
        new_co = update_apply[idx]
        if not np.isnan(new_co).any():
            v.co = new_co
        else:
            # NaN이 포함된 경우 업데이트 생략
            pass
    ...
    current_direct_v = inst.direct_u * (inst.stack+1)[:,None] * (base_ob.MKVBS.learning_rate)
    new_direct_v = np.random.randn(inst.vc,3)
    new_direct_u = new_direct_v / np.sqrt(np.einsum('ij,ij->i',new_direct_v,new_direct_v))[:,None]
    new_direct_v = new_direct_u * (inst.stack+1)[:,None] * base_ob.MKVBS.learning_rate
